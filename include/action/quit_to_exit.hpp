...
void quit_to_exit(ENetEvent event, const std::string& header) 
{
    ...
    peers(ENET_PEER_STATE_CONNECTED, [&](ENetPeer& p) 
    {
        ...
        {
            gt_packet(p, false, "OnConsoleMessage", std::format("`5<`w{}`` left, `w{}`` others here>``", _peer[event.peer]->ltoken[0], worlds[_peer[event.peer]->recent_worlds.back()].visitors).c_str());
