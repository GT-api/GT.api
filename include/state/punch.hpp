    ...
    if (state.id == 18) // @note punching a block
    {
        ...
        if (b.fg == 8 or b.fg == 6)
        {
            gt_packet(*event.peer, false, {
                "OnTalkBubble", 
                ((unsigned)_peer[event.peer]->netid, b.fg == 8) ? "It's too strong to break." : "(stand over and punch to use)"
            });
            return;
        }
        ...
        if (rand() % 5 == 1) 
        {
            drop_visuals(event, 
                ...
                {
                    static_cast<float>(state.punch[0]) + static_cast<float>((rand() % 20 + 60) * 0.1f), 
                    static_cast<float>(state.punch[1]) + static_cast<float>((rand() % 20 + 60) * 0.1f)
                }
            );
        }
    }
